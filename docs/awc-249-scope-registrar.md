...
| I6  | Registrar never calls Linear/Gmail/Jane/network. `fetch` is not imported. | Static: no fetch/http imports. Runtime: canary + test *no external mutation* fail closed if `fetch` fires. |
...
5. **Enable dispatch (last).** Wire a Linear bridge behind
   `ticketDispatchEnabled`. Created issues land in `Todo`. Never trigger
   a sprint from a registrar-created ticket.
